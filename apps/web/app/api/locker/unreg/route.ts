...
export async function POST(
  ...
  try {
    ...
    if (!isUserVerified){
      return NextResponse.json(
        { error:  `User ${name} with uin ${uin} is not an owner for locker ${l_id}` }, 
        { status: 401 }
      );
    }
    ...
    if (w_new > w_empty){
      ...
      return NextResponse.json(
        { error: `Current weight ${weight}: All belongings haven't been cleared out of locker` }, 
        { status: 409 }
      );
    }
